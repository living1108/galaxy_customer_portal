...
read -p "Do you want to acquire new certificates from Let's Encrypt? [Y/n] " -i y -n 1 -r
echo
if [[ $REPLY =~ ^[Yy]?$ ]]; then
    ...
    echo "### Skipping Let's Encrypt certificate acquisition."
    echo "### Generating self-signed certificate for $NGINX_HOST ..."
    mkdir -p ./data/certbot/conf/live/$NGINX_HOST
    openssl req -x509 -nodes -days 365 -newkey rsa:4096 \
        -keyout ./data/certbot/conf/live/$NGINX_HOST/privkey.pem \
        -out ./data/certbot/conf/live/$NGINX_HOST/fullchain.pem \
